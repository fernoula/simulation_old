...
/*bool Simulator::RecBeam_SecProton(Double_t Qvalue,Track &Tr,vector<Int_t> IsProton){
  ...
  ///////////////////////////////////////////////////////
   E_p1_si = Tr.TrEvent.at(IsProton[0]).SiEnergy;
   E_p2_si = Tr.TrEvent.at(IsProton[1]).SiEnergy;

   d_p1 = Tr.TrEvent.at(IsProton[0]).PathLength; 
   d_p2 = Tr.TrEvent.at(IsProton[1]).PathLength; 

   theta_p1 = Tr.TrEvent.at(IsProton[0]).Theta;
   theta_p2 = Tr.TrEvent.at(IsProton[1]).Theta;

   phi_p1 = Tr.TrEvent.at(IsProton[0]).Phi;
   phi_p2 = Tr.TrEvent.at(IsProton[1]).Phi;

   E_p1_rxn = E_Loss_light->GetLookupEnergy(E_p1_si,(-d_p1));
   E_p2_rxn = E_Loss_light->GetLookupEnergy(E_p2_si,(-d_p2));
   
   E_p1_tot = E_p1_rxn + M_Light;
   E_p2_tot = E_p2_rxn + M_Light;

 
  //Total momentum of protons.
  P_p1 = sqrt(E_p1_tot*E_p1_tot - M_Light*M_Light);
  //Components of 4-vectors for proton.
  p1_x = P_p1*sin(theta_p1)*cos(phi_p1);
  p1_y = P_p1*sin(theta_p1)*sin(phi_p1);
  p1_z = P_p1*cos(theta_p1);

  P_p2 = sqrt(E_p2_tot*E_p2_tot - M_Light*M_Light);
  //Components of 4-vectors for proton.
  ...
  //cout << s1 << " " << s2 << " " << s2*s2 <<  " " << Qvalue << endl;

  Double_t Beam_KE = s2*s2;
  Double_t KE_20Ne = Beam_KE - E_p1_rxn - E_p2_rxn + Qvalue;

  Double_t Theta_20NeRecoil = 0.0;
  Double_t cosh = (2*ma*Beam_KE + 2*mB*KE_20Ne - (SUM_p1p2_Mag*SUM_p1p2_Mag))/(4*sqrt(ma*mB*Beam_KE*KE_20Ne));

  Theta_20NeRecoil = acos(cosh)*180/TMath::Pi();
  

 
  Tr.HeavyEvent.BeamEnergy_20Ne = Beam_KE; // correct Beam_KE
  Tr.HeavyEvent.KE_20Ne = KE_20Ne;
  Tr.HeavyEvent.Theta_20Ne = Theta_20NeRecoil;
